...
- `pnpm run docs` — build the packages, then the documentation site into `apps/docs/dist`; `pnpm
  docs:dev` serves it with live reload. Not `pnpm docs`: that is a pnpm built-in that opens a
  package homepage and never runs the script. The site's API reference is generated by TypeDoc from
  the TSDoc comments on every build (`apps/docs/src/content/docs/reference/`, gitignored), so a
  public symbol's doc comment *is* its reference page. Set `DOCS_SEARCH=false` on machines where
  Pagefind's native binary can't run (linux-arm64 with 16K/64K memory pages).

Documentation site (`apps/docs`):

- Guides and component pages are MDX in `src/content/docs/`; live examples are React components in
  `src/examples/`, embedded with `client:only="react"` and wrapped in `Demo` (which supplies the
  width). Link between pages with relative links, so the site works under any base path.
- A new component, prop or behaviour gets its page or section updated along with the code.
- `packages/components` fails its build if the bundle can't be imported and server-rendered in
  Node (`scripts/check-ssr.mjs`): the site pre-renders every page, and so do consumers' SSR apps.
